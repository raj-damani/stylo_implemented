...
class StyloClassifier(object):
	def __init__(self,corpus,num_train=-1,num_val=-1,pred_col='Author'):
		...
		self.res = dict(zip(self.data_frame['Author'].values, self.data_frame['Author_Orig'].values))
		self.pred_col = pred_col
		self.cols = [c for c in self.data_frame.columns if c not in (self.pred_col,'Title','Author_Orig', 'Review')]
...
class StyloDecisionTree(StyloClassifier):
	...
	def predict(self,corpus=None):
		is_df = False
		if not isinstance(corpus, pd.DataFrame) and not corpus:
		...
		data = []
		for i in range(self.ypred.size):
			data.append(self.res.get(self.ypred[i],""))
		self.ypred = np.array(data)
		return self.ypred
